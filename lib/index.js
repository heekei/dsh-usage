...
const INTERVAL_MS = () => Number(process.env.DSH_USAGE_INTERVAL_MS || 300000);
...
export function apply(ctx, config = {}) {
  ...
  ctx.effect(() => () => clearInterval(timer), "dsh-usage: refresh timer");
  log("dsh-usage 插件已启动 (命名空间 dsh-usage, 每 " + INTERVAL_MS() / 1000 + "s 刷新)");
}
