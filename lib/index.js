...
// pi-ai 内置 catalog 路由 (kimi-coding / deepseek): 不写进 settings.yaml 也存在
// (deepseek 是 DSH 默认模型). usage 插件按 settings.yaml 枚举供应商, 这里补兜底,
// 否则内置 deepseek 永远不被查询. 默认凭据引用与 pi-ai 官方 catalog 一致.
const CATALOG_ROUTES = ["kimi-coding", "deepseek"];
const CATALOG_API_KEY_ENV = { "kimi-coding": "KIMI_API_KEY", "deepseek": "DEEPSEEK_API_KEY" };
const CATALOG_DEFAULTS = {
  "kimi-coding": { type: "kimi", base: "Kimi For Coding", apiKeyEnv: "KIMI_API_KEY" },
  "deepseek": { type: "deepseek", base: "DeepSeek", apiKeyEnv: "DEEPSEEK_API_KEY" },
};
...
function readCredentials() {
  ...
  // 兼容两种格式:
  //   v1:  version: 1 / refs: /   DEEPSEEK_API_KEY: sk-xxx   (DSH 原生, 键缩进在 refs 下)
  //   旧:  DEEPSEEK_API_KEY: sk-xxx                          (列 0 平铺)
  let inRefs = false;
  for (const line of readFileSync(CREDS(), "utf8").split(/\r?\n/)) {
    const t = line.trim();
    if (t === "" || t.startsWith("#")) continue;
    if (/^(version|refs|records):/.test(t)) { inRefs = t.startsWith("refs:"); continue; }
    const m = t.match(/^([A-Z_][A-Z0-9_]*):\s*(.*)$/);
    if (!m) continue;
    if (!inRefs && !/^\S/.test(line)) continue; // 非 refs 段的缩进键 (如 records 内) 不收
    creds[m[1]] = m[2].replace(/^["']|["']$/g, "").replace(/\s*#.*$/, "").trim();
  }
  return creds;
}
...
function buildSpecs(config, providers) {
  ...
  for (const [route, cfg] of Object.entries(configured)) {
    ...
    const spec = {
      ...
      apiKeyEnv: cfg.apiKeyEnv || entry.apiKeyEnv || CATALOG_API_KEY_ENV[route] || "",
      ak: cfg.accessKeyId || "",
  ...
  //    显式配置过 (含 enabled:false / type:none) 的路由跳过, 否则会被自动推断复活.
  for (const [route, entry] of Object.entries(providers || {})) {
    if (specs[route]) continue;
    if (configured[route] !== undefined) continue;
    const type = detectType(entry.baseURL, route);
    if (!type) continue;
    specs[route] = { type, base: entry.displayName || route, apiKeyEnv: entry.apiKeyEnv || CATALOG_API_KEY_ENV[route] || "", ak: "", sk: "" };
  }
  // 3) pi-ai 内置 catalog 路由兜底: yaml 未显式写出、也未在配置里显式处理时才补上
  //    (deepseek 是默认模型; kimi-coding 同机制). 显式 none/disabled 会尊重关闭.
  for (const [route, def] of Object.entries(CATALOG_DEFAULTS)) {
    if (specs[route]) continue;
    if (configured[route] !== undefined) continue;
    specs[route] = { type: def.type, base: def.base, apiKeyEnv: def.apiKeyEnv, ak: "", sk: "" };
  }
  return specs;
}
...
export async function refresh(configOverride) {
  ...
  // 内置 catalog 路由补进路由表: 否则 providers[route] 缺失, 查询会被拒
  for (const route of CATALOG_ROUTES) if (!(route in providers)) providers[route] = {};
