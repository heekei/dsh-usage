window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function UsageConfigSection() {
			const [view, setView] = react.useState({ status: "loading", error: null, routes: [], config: {}, secrets: [], revision: null, saving: false, feedback: null });
			...
			const load = react.useCallback(() => {
				...
				rpc("settings.describe", {}).then((r) => {
					...
					setDraft({});
					setView({ status: "ready", error: null, routes, config, secrets, baseURLs, revision: usage ? usage.revision : null, saving: false, feedback: null, usageMissing: !usage });
